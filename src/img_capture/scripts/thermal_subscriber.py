...
def filter_image(img, first_threshold, filter_boost, filter_sigma, second_threshold, camera_dims):
    """
    img is now in [0..100] float range.
    ...
    5) Produce visual_img (white=100 where >0, black=0 otherwise)
    ...
    # 5) Make a visual copy: anything >0 set to 100 for display
    ...
    # Return (visual_img in [0..100], hot_spots, weights)
    return visual_img, np.array(hot_spots).astype(int), np.array(weights)
...
class ThermalImageSubscriber(Node):
    """
    ROS 2 node that subscribes to 'camera/thermal_image' (mono16).
    ...
    def listener_callback(self, msg):
        ...
        # 2) Scale to [0..100]
        #   The raw range is [0..65535].
        #   We'll convert to float in [0..100].
        scaled_img = (img_array.astype(float) / 65535.0) * 100.0

        self.current_image = scaled_img  # store the float array
        self.update_requested = False
...
# -------------------------
def main():
    # 1) Load or create parameter file (in [0..100] range)
    ...
    if os.path.exists(param_file):
        ...
        # create with new defaults
        params = np.array([10, 10, 2, 20], dtype=float)
        np.save(param_file, params)

    detection_first_threshold  = params[0]  # e.g. 10
    detection_filter_boost     = params[1]  # e.g. 10
    detection_filter_sigma     = params[2]  # e.g. 2
    detection_second_threshold = params[3]  # e.g. 20

    # 2) Initialize ROS and spin in background
    rclpy.init()
    ...
    root = tk.Tk()
    root.title("Thermal Viewer: Pre-AGC 16-bit -> [0..100] float")

    fig = Figure(figsize=(8,4), dpi=100)
    (ax1, ax2) = fig.subplots(1,2)

    # Dummy data (float in [0..100])
